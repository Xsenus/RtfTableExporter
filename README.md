...
Кроссплатформенная консольная утилита на .NET для быстрого экспорта данных из `RTF` и `DOCX` в `TXT`.

Программа автоматически определяет поддерживаемую форму отчета. Для формы `0531857` берутся две табличные секции с данными по порядку документа: сначала доходы, затем расходы. Для формы `0503152` берется единая таблица отчета. Если форма не распознана, программа выгружает все найденные таблицы как обычный текст.

## Что умеет

- Если входные пути не переданы, программа обрабатывает все `*.rtf` и `*.docx`, которые лежат рядом с исполняемым файлом.
- Автоматически обрабатывает поддерживаемые формы отчетов `0531857` и `0503152`.
- Для неизвестных форм включает общий режим: берет все содержательные таблицы по порядку документа и пишет их строки как есть.
- Старый бинарный формат `.doc` напрямую не читается; такой файл нужно сохранить как `.docx` или `.rtf`.
